...
#I did not have the money to use open ai model for the llms so it was mandaotry for me to use 
#the same model for both writer adn reviewer model , but htis step is done just to make sure I properly understand the 
#concept of multiple llms being used for differnet cases according to their strengths
...
def extract_draft_node(state:State)->dict:
    """After the writer node finishes tool calls,pulls the final text out as the draft."""
...
def reveiwer_node(state:State)->dict:
...
graph.add_node("reviewer",reveiwer_node)
